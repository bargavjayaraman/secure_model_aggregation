...
# supporting intermediate output
# different machines hold different number of data
# Therefore we need individual lambda1/lambda2 for each machine
# input data 2 * m list
distLDA_asym2 <- function(data, lambda1, lambda2, t, isInDetail = FALSE, doTruncateForNaive = FALSE){
  #n = N / m
  #n2 = n - n1
  m = length(data) / 2
  d = nrow(data[[1]])
  ...
  if(isInDetail){
    betatilde = betatilde %*% diag(n1_vec + n2_vec) * m / sum(n1_vec + n2_vec)
    betahat = betahat %*% diag(n1_vec + n2_vec) * m / sum(n1_vec + n2_vec)
